...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 31 23:14:46 2017
...
def value_save(val1, val2, val3, val4, domain1, domain2, domain3, domain4):
    ...
    index = min(data)
    print (index)
    path = '/Users/JayBaek/Desktop/energy_value_walking.csv'
    remove_file(path)
    for i in range (index):
        saveFile = open('energy_value_walking.csv', 'a')
...
def avg_value_save(val1, val2, val3, val4, domain1, domain2, domain3, domain4):
    data = [len(domain1), len(domain2), len(domain3), len(domain4)]
    index = min(data)
    print (index)
    path = '/Users/JayBaek/Desktop/normalized_energy_value.csv'
    remove_file(path)
    if os.path.exists(path):
        os.remove(path)
    for i in range (index):
        saveFile = open('normalized_energy_value.csv', 'a')
        saveFile.write(str(val1[i]) + ',' + str(val2[i]) + ',' + str(val3[i]) + ',' + str(val4[i]))
        saveFile.write('\n')
    saveFile.close()
    data_path = '/Users/JayBaek/Desktop/normalized_energy_value.csv'
...
avg_value_save(energy_nor, energy_nor1, energy_nor2, energy_nor3, freq_domain, freq_domain1, freq_domain2, freq_domain3)
